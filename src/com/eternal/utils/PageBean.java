...
/**
 * 分页的三个基本属性 1.每页几条记录size 可以有默认值5 2.当前页号 index 可以有默认值1
 * 3.记录总数totalCount：不可能有默认值，需要查询数据库获取真正的记录总数
 * 
 * 4.一共多少页 ：totalPageCount=totalCount/size+1 5 30 31 32 33 34 35 5.上一页 index-1
 * 当前页1，上一页1 6.下一页 index+1 当前页是最后一页 下一页：还是最后一页
 * 
 * 扩展 分页Bean还可以放要查询的数据 protected List<T> list; 分页Bean还可以放页码列表 [1] 2 3 4 5
 * private int[] numbers;
 * 
