...
import com.eternal.service.impl.EmployeeServiceImpl;
...
public class EmployeeServlet extends BaseServlet {
	...
	public void upDate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Date user_addtime =
		// java.sql.Date.valueOf(request.getParameter("user_addtime"));
		...
		UserInfo ui = new UserInfo(user_id, user_name, user_age, user_sex, user_nation, user_diploma, is_married,
				department_id, role_id, user_tel, user_intest, user_bankcard, user_mobile, user_idnum, user_addtime,
				user_addman, user_changeman, user_email, user_address);
		...
		if (n > 0) {
			...
			request.getRequestDispatcher("/Estate/view/employee/emp_edit.jsp").forward(request, response);
		}
	}
	...
	public void delete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (n > 0) {
			PrintWriter out = response.getWriter();
			out.println(
					"<script type='text/javascript'>alert('删除成功');window.location.href='/Estate/servlet/EmployeeServlet?method=find';</script>");
		} else {

			PrintWriter out = response.getWriter();
			out.println(
					"<script type='text/javascript'>alert('删除失败');window.location.href='/Estate/servlet/EmployeeServlet?method=find';</script>");
		}

	}

	/**
	 * 用户登录的方法，登录时判断邮箱是否验证，没有验证则不能登录 同时增加利用cookie记住密码功能
	 * 
	 * @author wy
	 */
		...
		if (user == null) {
			...
			if ("actived".equals(is_used)) {
				...
				String url = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
						+ request.getContextPath() + "/"; // 相当于 :http://127.0.0.1:8181/Estate/
				request.setAttribute("activeerror",
						"您还未激活无法登录,点击<a style='color:#ff0000;' href='"+url+"servlet/AdminServlet?method=sendEmail&email="
								+ user.getUser_email() + "'>发送激活邮件。</a>");
				request.getRequestDispatcher("/login.jsp").forward(request, response);
			}
		}
	}
